function [pks,locs] = findpeaksIBEI(X,varargin)
...
%   $Revision: 1.1.6.1 $  $Date: 2007/05/23 19:11:57 $
...
elseif Ph == -Inf
    ...
    if(L==0)
        ...
        error(mphmsgid,'Invalid MinPeakHeight. There are no data points greater than MinPeakHeight.')
    else
        % validate value of Pd and set default values for Pd and Np
        [Pd,Np] = setvalues(Pd,Np,L); 
        if(Pd >=Indx(L))
...
while (j < L) && (m < Np)
    ...
    if Pd > 1
        ...
        if (all(Data(j) > (temp([1:idx1,idx2:end])+Th)) && all(Data(j) >= temp(idx3:idx4)))
            m = m+1;
            pks(m)  = Data(j);
            locs(m) = Indx(j);
        end
...
while (j < L) && (m < Np)
    ...
    if Pd > 1
        ...
        idx3 = max(1,aa-1);
        idx4 = min(aa+1,length(temp));
%         if(all((Data(j) > temp([1:idx1,idx2:end])+Th)) && all((Data(j) >= temp(idx3:idx4))) && (j ~=1) && j~=L)
        if (all(Data(j) > (temp([1:idx1,idx2:end])+Th)) && all(Data(j) >= (temp(idx3:idx4))))
            m = m+1;
            pks(m)  = Data(j);
            locs(m) = j;
